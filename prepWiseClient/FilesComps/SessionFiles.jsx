...
const SessionFiles = ({
    ...
    try {
      ...
      if (selectedFile.file) {
        ...
        const res = await fetch(
          `${apiUrlStart}/api/Files/UploadSessionFile?sessionId=${sessionId}&saveToFileList=false`,
          {
            method: "POST",
            headers: {
  ...
  const uploadSessionFile = async (file, saveToProfile = true) => {
    try {
      ...
      const uploadUrl = `${apiUrlStart}/api/Files/upload-file?userId=${Loggeduser.id}&sessionId=${sessionId}&saveToFileList=${saveToProfile}`;
  ...
  const fetchSessionFiles = async () => {
    try {
      const response = await fetch(
        `${apiUrlStart}/api/Files/GetSessionFiles?sessionId=${sessionId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const deleteSessionFile = async (fileId) => {
    try {
      const response = await fetch(
        `${apiUrlStart}/api/Files/DeleteSessionFile?sessionId=${sessionId}&fileId=${fileId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <View>
      ...
      <Text style={appliedStyles.subtitlesmall}>
        Want to share a resume, portfolio, or notes before the session? Upload
        here — you can always add more later.
      </Text>
      ...
      {attachedFiles.length > 0 && (
        <View style={{ marginTop: 10 }}>
          <Text
            style={[appliedStyles.subtitle, { fontSize: 16, marginBottom: 10 }]}
          >
